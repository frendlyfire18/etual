    ...
    return (_parent, fieldArgs, cache, info) => {
        ...
        if(info.variables.brand){
            fieldInfos.forEach((fi,value)=>{
                ...
                if(!_hasMore)
                    hasMore = _hasMore as boolean;
                if(fi.fieldKey.indexOf(info.variables.brand as string) !== -1){
                    result = []
                    result.push(...res);
                }
            })
        }else if(info.variables.priceRange !== undefined && info.variables.secondRange !== undefined){
            fieldInfos.forEach((fi,value)=>{
                const key = cache.resolveFieldByKey(entityKey,fi.fieldKey) as string;
                const res = cache.resolve(key,"records") as string[];
                const _hasMore = cache.resolve(key,"hasMore");
        ...
        }else if(info.variables.country === undefined && info.variables.forWho === undefined && info.variables.brand === undefined){
...
export const CreateURQLClient = (ssrExchange:any,ctx:any)=>{
    ...
    return {
        url: 'https://etual-server.herokuapp.com/graphql',//url of graphql server
        fetchOptions:{
